package com.example.specialeffectsandroid3.metaball;
...
public class MetaballView extends View {
	...
	private ArrayList<Circle> circlePaths = new ArrayList<MetaballView.Circle>();
	...
	private void init() {
		...
		circlePath.center = new float[] { (radius + ITEM_DIVIDER),
				radius * (1f + SCALE_RATE) };
		circlePath.radius = radius / 4 * 3;
		circlePaths.add(circlePath);

		for (int i = 1; i < ITEM_COUNT; i++) {
			circlePath = new Circle();
			circlePath.center = new float[] { (radius * 2 + ITEM_DIVIDER) * i,
					radius * (1f + SCALE_RATE) };
	 ...
	 * @param maxDistance
	 */
	private void metaball(Canvas canvas, int j, int i, float v,
			float handle_len_rate, float maxDistance) {
		...
		float[] center1 = new float[] { ball1.centerX(), ball1.centerY() };
		float[] center2 = new float[] { ball2.centerX(), ball2.centerY() };
		...
		if (d > maxDistance) {
			// canvas.drawCircle(ball1.centerX(), ball1.centerY(),
			// circle1.radius, paint);
			canvas.drawCircle(ball2.centerX(), ball2.centerY(), circle2.radius,
					paint);
		} else {
			...
			radius2 *= scale2;
			// radius1 *= scale1;
			// canvas.drawCircle(ball1.centerX(), ball1.centerY(), radius1,
			// paint);
			canvas.drawCircle(ball2.centerX(), ball2.centerY(), radius2, paint);

		}

		// Log.d("Metaball_radius", "radius1:" + radius1 + ",radius2:" +
		// radius2);
		...
		if (d > maxDistance || d <= Math.abs(radius1 - radius2)) {
			...
			u1 = (float) Math.acos((radius1 * radius1 + d * d - radius2
					* radius2)
					/ (2 * radius1 * d));
			u2 = (float) Math.acos((radius2 * radius2 + d * d - radius1
					* radius1)
					/ (2 * radius2 * d));
		} else {
			u1 = 0;
		...
		// Log.d("Metaball", "center2:" + Arrays.toString(center2) + ",center1:"
		// + Arrays.toString(center1));
		float[] centermin = new float[] { center2[0] - center1[0],
				center2[1] - center1[1] };
		...
		float angle2a = (float) (angle1 + Math.PI - u2 - (Math.PI - u2 - angle2)
				* v);
		float angle2b = (float) (angle1 - Math.PI + u2 + (Math.PI - u2 - angle2)
				* v);

		// Log.d("Metaball", "angle1:" + angle1 + ",angle2:" + angle2 +
		// ",angle1a:" + angle1a + ",angle1b:" + angle1b + ",angle2a:" + angle2a
		// + ",angle2b:" + angle2b);
		...
		// Log.d("Metaball", "p1a:" + Arrays.toString(p1a) + ",p1b:" +
		// Arrays.toString(p1b) + ",p2a:" + Arrays.toString(p2a) + ",p2b:" +
		// Arrays.toString(p2b));
		...
		float d2 = Math
				.min(v * handle_len_rate, getLength(p1_p2) / totalRadius);
		...
		// Log.d("Metaball", "sp1:" + Arrays.toString(sp1) + ",sp2:" +
		// Arrays.toString(sp2) + ",sp3:" + Arrays.toString(sp3) + ",sp4:" +
		// Arrays.toString(sp4));

		Path path1 = new Path();
		path1.moveTo(p1a[0], p1a[1]);
		path1.cubicTo(p1a[0] + sp1[0], p1a[1] + sp1[1], p2a[0] + sp2[0], p2a[1]
				+ sp2[1], p2a[0], p2a[1]);
		path1.lineTo(p2b[0], p2b[1]);
		path1.cubicTo(p2b[0] + sp3[0], p2b[1] + sp3[1], p1b[0] + sp4[0], p1b[1]
				+ sp4[1], p1b[0], p1b[1]);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(ball1.centerX(), ball1.centerY(), circle.radius,
				paint);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		setMeasuredDimension(
				resolveSizeAndState(
						(int) (ITEM_COUNT * (radius * 2 + ITEM_DIVIDER)),
						widthMeasureSpec, 0),
				resolveSizeAndState((int) (2 * radius * 1.4f),
						heightMeasureSpec, 0));
	}
