...
/**
 * Created by chenupt@gmail.com on 2015/1/31. Description : Draw a head point
 * and foot point.
 */
public class SpringView extends View {
	...
	private void makePath() {
		...
		float headOffsetX = (float) (headPoint.getRadius() * Math.sin(Math
				.atan((footPoint.getY() - headPoint.getY())
						/ (footPoint.getX() - headPoint.getX()))));
		float headOffsetY = (float) (headPoint.getRadius() * Math.cos(Math
				.atan((footPoint.getY() - headPoint.getY())
						/ (footPoint.getX() - headPoint.getX()))));

		float footOffsetX = (float) (footPoint.getRadius() * Math.sin(Math
				.atan((footPoint.getY() - headPoint.getY())
						/ (footPoint.getX() - headPoint.getX()))));
		float footOffsetY = (float) (footPoint.getRadius() * Math.cos(Math
				.atan((footPoint.getY() - headPoint.getY())
						/ (footPoint.getX() - headPoint.getX()))));
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(headPoint.getX(), headPoint.getY(),
				headPoint.getRadius(), paint);
		canvas.drawCircle(footPoint.getX(), footPoint.getY(),
				footPoint.getRadius(), paint);
		super.onDraw(canvas);
	}

	public void animCreate() {
		ViewHelper.setPivotX(this, getHeadPoint().getX());
		ViewHelper.setPivotY(this, getFootPoint().getY());
