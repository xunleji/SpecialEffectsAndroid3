...
public class SpringIndicator extends FrameLayout {
	...
	private void createPoints() {
		...
		springView.getHeadPoint().setX(
				ViewHelper.getX(view) + view.getWidth() / 2);
		springView.getHeadPoint().setY(
				ViewHelper.getY(view) + view.getHeight() / 2);
		springView.getFootPoint().setX(
				ViewHelper.getX(view) + view.getWidth() / 2);
		springView.getFootPoint().setY(
				ViewHelper.getY(view) + view.getHeight() / 2);
		...
		viewPager
				.setOnPageChangeListener(new ViewPager.SimpleOnPageChangeListener() {
					...
					public void onPageScrolled(int position,
						...
						if (position < tabs.size() - 1) {
							...
							if (positionOffset < radiusOffsetFoot) {
								springView.getFootPoint().setRadius(
										(1 - positionOffset / radiusOffsetFoot)
												* radiusOffset + radiusMin);
							} else {
							...
							if (positionOffset < headMoveOffset) {
								float positionOffsetTemp = positionOffset
										/ headMoveOffset;
								headX = (float) ((Math.atan(positionOffsetTemp
										* acceleration * 2 - acceleration) + (Math
										.atan(acceleration))) / (2 * (Math
										.atan(acceleration))));
							}
							springView.getHeadPoint().setX(
									getTabX(position) - headX
											* getPositionDistance(position));
							float footX = 0f;
							if (positionOffset > footMoveOffset) {
								float positionOffsetTemp = (positionOffset - footMoveOffset)
										/ (1 - footMoveOffset);
								footX = (float) ((Math.atan(positionOffsetTemp
										* acceleration * 2 - acceleration) + (Math
										.atan(acceleration))) / (2 * (Math
										.atan(acceleration))));
							}
							springView.getFootPoint().setX(
									getTabX(position) - footX
											* getPositionDistance(position));
						...
						if (delegateListener != null) {
							delegateListener.onPageScrolled(position,
	...
	private float getTabX(int position) {
		return ViewHelper.getX(tabs.get(position))
				+ tabs.get(position).getWidth() / 2;
	}
	...
	private void setSelectedTextColor(int position) {
		...
		tabs.get(position).setTextColor(
				getResources().getColor(selectedTextColorId));
	}
