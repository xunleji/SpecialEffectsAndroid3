...
/**
 * BitmapMergerTask is background asynchronous task merging two bitmaps by
 * placing one bitmap (called mergeBitmap) over another (called baseBitmap).
 * 
 * Merging can be done in three ways,
 * 
 * 1. Merging at the center (along with resize options) 2. Merging at an angle
 * away from the axis of the image at center point 3. Merging the bitmaps with
...
public class BitmapMergerTask extends AsyncTask<Void, Void, Bitmap> {
	...
	public enum BitmapMergeOptions {
		MERGE_AT_CENTER, MERGE_AT_ANGLE_OFF, MERGE_FROM_TOP_LEFT,
	}
	 ...
	 * 
	 * @param scale
	 *            - float value from 0.0 to 1.0 represents the scale.
	...
	/**
	 * Sets the merging offset points. Invoking this method will mark the
	 * merging mechanism to merge the mergeBitmap image to the base bitmap image
	 * from the top left portion as specified by the params leftOffset and
	 * topOffset
	 * 
	 * @param leftOffset
	 *            pixel offsets from left
	 * @param topOffset
	 *            pixel offsets from top
	 * @return the related BitmapMergerTask
	 */
	...
	/**
	 * Sets the merging offset angle. Invoking this method will mark the merging
	 * mechanism to merge the mergeBitmap image to the base bitmap image at the
	 * angle off from the base line from center to mid point on the right edge
	 * of the baseBitmap.
	 * 
	 * @param angle
	 *            - angle off from the base line.
	 * @return the related BitmapMergerTask
	 */
	...
	private Bitmap mergeAtAngle() {
		...
		if (mScale > 0) {
			...
			Bitmap overlayScaled = Bitmap.createScaledBitmap(mMergeBitmap,
					(int) (mBaseBitmap.getWidth() * mScale),
					(int) (mBaseBitmap.getHeight() * mScale), true);
	...
	private Bitmap mergeFromTopLeft() {
		...
		return mergeBitmaps(mBaseBitmap, mMergeBitmap, mScale, mLeftOffset,
				mTopOffset);
	}
	...
	private Bitmap mergeAtCenter() {
		...
		if (mScale > 0) {
			...
			return mergeBitmaps(mBaseBitmap, mMergeBitmap, mScale, startX,
					startY);
		} else {
			return mBaseBitmap;
		}
	...
	private static Bitmap mergeBitmaps(Bitmap baseBitmap, Bitmap overlayBitmap,
			float scale, int leftOffset, int topOffset) {

		if (scale > 0) {
			Bitmap overlayScaled = Bitmap.createScaledBitmap(overlayBitmap,
					(int) (baseBitmap.getWidth() * scale),
					(int) (baseBitmap.getHeight() * scale), true);
