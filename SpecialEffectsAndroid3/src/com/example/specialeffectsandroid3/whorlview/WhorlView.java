...
public class WhorlView extends View {
	...
	public WhorlView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (attrs != null) {
			final TypedArray typedArray = context.obtainStyledAttributes(attrs,
					R.styleable.whorlview_style);
			String colors = typedArray
					.getString(R.styleable.whorlview_style_whorlview_circle_colors);
			if (TextUtils.isEmpty(colors)) {
				colors = defaultColors;
			}
			parseStringToLayerColors(colors);
			mCircleSpeed = typedArray.getInt(
					R.styleable.whorlview_style_whorlview_circle_speed,
					defaultCircleSpeed);
			int index = typedArray.getInt(
					R.styleable.whorlview_style_whorlview_parallax, 0);
			setParallax(index);
			mSweepAngle = typedArray.getFloat(
					R.styleable.whorlview_style_whorlview_sweepAngle,
					defaultSweepAngle);
			...
			mStrokeWidth = typedArray.getFloat(
					R.styleable.whorlview_style_whorlview_strokeWidth,
					defaultStrokeWidth);
			typedArray.recycle();
		} else {
		...
		for (int i = 0; i < colorArray.length; i++) {
			try {
				...
				throw new IllegalArgumentException(
						"whorlview_circle_colors can not be parsed | "
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < mLayerColors.length; i++) {
			float angle = (mCircleSpeed + mParallaxSpeed * i) * mCircleTime
					* 0.001f;
	...
	private RectF checkRectF(int index) {
		...
		float start = index * (mStrokeWidth + mIntervalWidth) + mStrokeWidth
				/ 2;
	...
	private void calculateIntervalWidth(int size) {
		float wantIntervalWidth = (size / (mLayerColors.length * 2))
				- mStrokeWidth;
		// 防止间隔太大，最大为弧宽的3倍
