...
public class DrawerArrowDrawable extends Drawable {
	...
	/**
	 * Joins two {@link Path}s as if they were one where the first 50% of the
	 * path is {@code PathFirst} and the second 50% of the path is
	 * {@code pathSecond}.
	 */
	private static class JoinedPath {
		...
		/**
		 * Returns a point on this curve at the given {@code parameter}. For
		 * {@code parameter} values less than .5f, the first path will drive the
		 * point. For {@code parameter} values greater than .5f, the second path
		 * will drive the point. For {@code parameter} equal to .5f, the point
		 * will be the point where the two internal paths connect.
		 */
	...
	/**
	 * Draws a line between two {@link JoinedPath}s at distance
	 * {@code parameter} along each path.
	 */
	private class BridgingLine {
		...
		/**
		 * Draw a line between the points defined on the paths backing
		 * {@code measureA} and {@code measureB} at the current parameter.
		 */
		...
		/**
		 * Insets the end points of the current line to account for the
		 * protruding ends drawn for {@link Cap#ROUND} style lines.
		 */
	...
	/**
	 * Paths were generated at a 3px/dp density; this is the scale factor for
	 * different densities.
	 */
	private final static float PATH_GEN_DENSITY = 3;

	/**
	 * Paths were generated with at this size for
	 * {@link DrawerArrowDrawable#PATH_GEN_DENSITY}.
	 */
	private final static float DIMEN_DP = 23.5f;

	/**
	 * Paths were generated targeting this stroke width to form the arrowhead
	 * properly, modification may cause the arrow to not for nicely.
	 */
	...
	public void draw(Canvas canvas) {
		if (flip) {
			canvas.save();
			canvas.scale(1f, -1f, getIntrinsicWidth() / 2,
	...
	/**
	 * Sets the rotation of this drawable based on {@code parameter} between 0
	 * and 1. Usually driven via
	 * {@link DrawerListener#onDrawerSlide(View, float)}'s {@code slideOffset}
	 * parameter.
	 */
	...
	/**
	 * When false, rotates from 3 o'clock to 9 o'clock between a drawer icon and
	 * a back arrow. When true, rotates from 9 o'clock to 3 o'clock between a
	 * back arrow and a drawer icon.
	 */
	 ...
	 * {@link DrawerArrowDrawable#PATH_GEN_DENSITY}, no scaling needs to be
	 * done.
	 */
	private static void scalePath(Path path, float density) {
		...
		scaleMatrix.setScale(density / PATH_GEN_DENSITY, density
				/ PATH_GEN_DENSITY, 0, 0);
