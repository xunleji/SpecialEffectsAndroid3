...
public class LeftDrawerLayout extends ViewGroup {
		...
		mHelper = ViewDragHelper.create(this, 1.0f,
				new ViewDragHelper.Callback() {
					...
					public void onViewReleased(View releasedChild, float xvel,
						...
						float offset = (childWidth + releasedChild.getLeft())
								* 1.0f / childWidth;
						boolean openMark = xvel > 0 || xvel == 0
								&& offset > 0.5f;
						mHelper.settleCapturedViewAt(
								openMark ? 0 : -childWidth,
					...
					public void onViewPositionChanged(View changedView,
						...
						Diary.print("offset=="+offset);
						if(onpos!=null){
							onpos.onPoschange((float)offset);
						}
						showShadow(offset);
						changedView.setVisibility(offset == 0 ? View.INVISIBLE
								: View.VISIBLE);
						...
						if (!releasing) {
							mFlowingView.show(rightX, pointY,
									FlowingView.Status.STATUS_UP);
						} else {
							mFlowingView.show(rightX, pointY,
									FlowingView.Status.STATUS_DOWN);
							if (rightX == 0) {
								mFlowingView.resetStatus();
								releasing = false;
							}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		MarginLayoutParams lp = (MarginLayoutParams) leftMenuView
				.getLayoutParams();
		...
		final int contentWidthSpec = MeasureSpec.makeMeasureSpec(widthSize
		...
		MarginLayoutParams mlp = (MarginLayoutParams) fragmentLayout
				.getLayoutParams();
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		MarginLayoutParams lp = (MarginLayoutParams) contentView
				.getLayoutParams();
		contentView.layout(lp.leftMargin, lp.topMargin, lp.leftMargin
	...
	public void closeDrawer() {
		...
		releasing = true;
		mFlowingView.resetContent();
		mHelper.smoothSlideViewTo(menuView, -menuView.getWidth(),
				menuView.getTop());
		postInvalidate();
	}
	...
	private OnPositionChange onpos;
	public void setPosChange(OnPositionChange onpos){
		this.onpos = onpos;
	}
}