...
public class FlowingView extends View {
	...
	public enum Status {
		NONE, STATUS_SMOOTH_UP, STATUS_UP, STATUS_DOWN,
	}
	...
	private void drawBG(Canvas canvas) {
		...
		case STATUS_UP:
			...
			if (currentPointY - getHeight() / 2 >= 0) {
				topY = (int) (currentPointY - 1.5 * currentPointX * getHeight()
						/ getWidth())
						- currentPointY / 2 + getHeight() / 4;
				bottomY = (int) (currentPointY + 1.5 * currentPointX
						* getHeight() / getWidth());
			} else {
				topY = (int) (currentPointY - 1.5 * currentPointX * getHeight()
						/ getWidth());
			...
			mPath.cubicTo(getWidth() - currentPointX, currentPointY / 4 + 3
					* topY / 4, getWidth(), 3 * currentPointY / 4 + topY / 4,
					getWidth(), currentPointY);
			mPath.cubicTo(getWidth(), 5 * currentPointY / 4 - topY / 4,
					getWidth() - currentPointX, 7 * currentPointY / 4 - 3
							* topY / 4, getWidth() - currentPointX, bottomY);
	...
	public void downing() {
		...
		ValueAnimator valueAnimator = ValueAnimator.ofInt(w + 100, w
				- rightMargin);
		valueAnimator
				.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
					@Override
					public void onAnimationUpdate(ValueAnimator animation) {
						currentPointX = (Integer) animation.getAnimatedValue();
